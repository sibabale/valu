...
const StockPill = styled(TouchableOpacity)<{ isSelected: boolean }>`
  padding: 4px 12px;
  border-radius: 20px;
  background-color: ${(props: { isSelected: boolean }) => props.isSelected ? '#3b82f6' : 'transparent'};
  border: 1px solid ${(props: { isSelected: boolean }) => props.isSelected ? '#3b82f6' : '#d1d5db'};
`;

const StockText = styled.Text<{ isSelected: boolean }>`
  font-size: 12px;
  font-weight: 500;
  color: ${(props: { isSelected: boolean }) => props.isSelected ? '#ffffff' : '#374151'};
  font-family: 'Space Grotesk';
`;
    ...
    <Container>
      ...
      <StocksContainer>
        {popularStocks.map(stock => (
          ...
          >
            <StockText isSelected={selectedStock === stock}>
              {stock}
            </StockText>
          </StockPill>
        ))}
